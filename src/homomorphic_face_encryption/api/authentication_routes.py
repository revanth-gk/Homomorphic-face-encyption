...
def authenticate():
    ...
    try:
        ...
        if not check_rate_limit(client_ip, limit=10, window_minutes=1):
            ...
            return jsonify({"error": "Rate limit exceeded: max 10 auth attempts per minute"}), 429
        ...
        try:
            ...
            return jsonify({"error": "Invalid base64 encoding for query embedding"}), 400
        ...
        if auth_mode == '1:1':
            ...
            if distance < threshold:
                ...
                return jsonify({
                    ...
                    "token": token,
                    "user_id": str(user_id),
                    "confidence_score": 1.0 - distance  # Convert distance to confidence
                }), 200
            else:
            ...
            try:
                ...
                if min_distance < threshold:
                    # Authentication successful
                    token = authenticate_user(min_user_id, data.get('device_fingerprint'))
                    ...
                    return jsonify({
                        ...
                        "confidence_score": 1.0 - min_distance  # Convert distance to confidence
                    }), 200
                else:
                    # Authentication failed
                    create_audit_log(
...
def async_compute_all_distances(query_embedding_binary: bytes, template_ids: list):
    ...
    """
    logger.info(f"Starting async distance computation for {len(template_ids)} templates")
    ...
    try:
        ...
        logger.info(f"Completed async distance computation for {len(results)} templates")
        return results
